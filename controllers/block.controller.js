...
const create = async (req, res) => {
    ...
    try {
        // Check existence;
        const exist = await blockModel.findOne({ name });
        if (exist) {
            return res.status(409).json({ err: 'Block already exists' })
        }

        const insert = await blockModel.create({ name, status, details });
...
const update = async (req, res) => {
    ...
    try {
        ...
        const result = await blockModel.updateOne({ _id: id }, { $set: { name, status, details } })
...
const get = async (req, res) => {
    ...
    try {
        ...
        if (id) {
            const data = await blockModel.findOne({ _id: id, isDel: "0" });
            if (!data) {
        ...
        const cacheKey = `Block:page=${page}:limit=${limit}`;
        ...
        const data = await blockModel.find({ isDel: "0" }).skip(skip).limit(limit).sort({ _id: -1 });
        const totalCount = await blockModel.countDocuments({ isDel: "0" });
